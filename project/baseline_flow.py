...
class BaselineNLPFlow(FlowSpec):
    ...
    def baseline(self):
        ...
        np.random.seed(44)
        self.baseline_predictions = np.random.randint(0, 2, size=self.valdf.shape[0])
    ...
    def end(self):
        ...
        current.card.append(Markdown ("## Examples of False Postives"))
        f = self.baseline_predictions == 1
        f &= self.valdf ['label'] == 0
        self.false_pos = self.valdf.where(f).dropna (how='all')
        current.card.append(Table. from_dataframe (self.false_pos[['review']].sample(5)))
        current.card.append(Markdown("## Examples of False Negatives"))
        f = self.baseline_predictions == 0
        f &= self.valdf ['label'] == 1
        self.false_neg = self.valdf.where(f).dropna (how='all')
